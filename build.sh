...
MOC_ARGS=$(< mops.sources) ## place args like compacting-gc, incremental-gc here
OUT=out/out_$(uname -s)_$(uname -m).wasm
moc src/main.mo -o $OUT -c -no-check-ir --release --public-metadata candid:service --public-metadata candid:args ${MOC_ARGS}
